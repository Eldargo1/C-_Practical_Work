...
* Для того что бы переминовать или изминить название переменной или функций которая повторяется во многих местах  например в 10 местах одинаковое название переменной что бы не изминять все в ручную например 10 раз нажимаю на название или имя переменной нажимаем правой кнопкой мыши и выбираем "Rename symbole".
 ## Запись с 6-го урока практики.Массивы и строки.

 * Тип перемнной char- в которой мы можем записать абсолутно любой символ но тлько один и плюс еще одна отличительная особенность закоючается в том что мы тут должны поставить одинарные ковычки если в string мы ставили двойные ковычки
 ```sh
char t = ' ';
 ```
* В чем разница массива char  и массива string - разница в том что в массиве string  мы не можем менять элементы а в char  мы можем.
* Можно заполнять массив char  в ручную как массив int
```sh
char [] array = new char[]{'t', '7', ';', 'p'};
```
 * Например значение переменного  типа char не можем перенести в тип данных string 
 ```sh
 char [] array = new char[]{'t', '7', ';', 'p'};
string str = ""
str = array[i] - так нельзя делать выйдет ошибка для этого надо 

str = str + array[i];

 ```
 * переменная типа var может подстраеваться под любой тип переменных (int.char,double и.т.д) используется в основном в цикле foreach.
 * Тип данных(String) является массивом и можно к ниму обращаться как к массиву

 * Функция Contains -  используется для определенной проверки например в заданий номер 3 (в папке Lesson6=>Task3) проверяем буквы на нласность
 * Отрицание в языке С# ставится знаком "!"
## Запись с 7-го урока. Рекурсия
* Функция Char.isLetter- проверяет является ли символ буквой  